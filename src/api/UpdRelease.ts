...
async function GetUpdate(
	...
	console.log(
		`Query: ${querynum} - Axios Query: ${
			new Date().getTime() - mainstarttime
		} ms`
	); // TIME LOGGER
	if (res.status == 200) {
		let resobj = xml2js(res.data.toString());
		console.log(
			`Query: ${querynum} - XML2js: ${
				new Date().getTime() - mainstarttime
			} ms`
		); // TIME LOGGER
		...
		console.log(
			`Query: ${querynum} - Removal of subsplease and erai raws: ${
				new Date().getTime() - mainstarttime
			} ms`
		); // TIME LOGGER
		//if (!sp)
		...
		console.log(
			`Query: ${querynum} - Removal of erai-raws if sp present and non-verified shit: ${
				new Date().getTime() - mainstarttime
			} ms`
		); // TIME LOGGER
		...
		console.log(
			`Query: ${querynum} - Removal of already watched shit: ${
				new Date().getTime() - mainstarttime
			} ms`
		); // TIME LOGGER
		...
		console.log(
			`Query: ${querynum} - Random for loop label: ${
				new Date().getTime() - mainstarttime
			} ms`
		); // TIME LOGGER
		let downloadlinks = [];
		...
		// }
		console.log(
			`Query: ${querynum} - Episode numbers: ${
				new Date().getTime() - mainstarttime
			} ms`
		); // TIME LOGGER
		...
		console.log(
			`Query: ${querynum} - Pushed all shit: ${
				new Date().getTime() - mainstarttime
			} ms`
		); // TIME LOGGER
