...
async function processScraper(job: Job<ScraperJobData>) {
    ...
    try {
        ...
        const imageResult = await db
            .select({ fileid: anime.fileid })
            .from(anime)
            .where(eq(anime.alid, alid));

        const imageFileId = imageResult[0]?.fileid;

        const chatIdsResult = await db
            .select({ userid: users.userid, chatid: users.chatid })
            .from(users)
        ...
        console.log(
            `Created ${chatIdsResult.length} notification jobs for ${jpname} Episode ${episode}`
        );
...
export const scraperWorker = new Worker<ScraperJobData>("episode-scraper", processScraper, {
