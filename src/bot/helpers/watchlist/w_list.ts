  ...
  return new Menu<MyContext>('wl_main').dynamic(async (ctx) => {
    ...
    for (let i = 0; i < wllist.length; i += 2) {
      ...
      range
        ...
        .submenu(
          wllist[i + 1].watchlist_name ?? '',
          `wl_opts`,
          async (ctx1) => {
            const chosenWLReplyMsg = fmt`Watchlist ${code}${wllist[i + 1].watchlist_name ?? ''}${code} chosen.\nWhat do you want to do with it?`;
            await ctx1.editMessageText(chosenWLReplyMsg.text, {
              entities: chosenWLReplyMsg.entities,
            });
            if (ctx1.session.menudata !== undefined)
...
export function animeList() {
  return new Menu<MyContext>('wl_allist').dynamic(async (ctx) => {
    ...
    if (ctx.session.menudata === undefined) {
      range.text('-Session expired. Generate a new menu.-');
      return range;
    }
    const movepg = ctx.session.menudata.l_page ?? 1;
    const temp = await getWlAlid(ctx, true, false);
    const towatch = ctx.session.menudata.listmethod === 'towatch';
    if (temp === 'back' || temp === undefined) {
      range.text('-Menu too old. Generate a new one.-');
      return range;
    }
    const wlid = temp.wlid;
    if (wlid === undefined) {
      range.text('-Error: No watchlist selected.-');
      return range;
    }
    const wlname = await getWLName(ctx);
    let maxpg: number | undefined = undefined;
    let wl: { jpname: string; enname: string; alid: number }[];
    if (ctx.session.menudata.maxpg === undefined) {
      const result = await getWatchlistAnime(wlid, movepg, 5, true, true, {
        towatch,
        userid:
          towatch === true ? (ctx.session.userid ?? undefined) : undefined,
      });
      if (result === undefined) {
        range.text('-Error fetching watchlist.-');
        return range;
      }
      wl = result.wl;
      maxpg = result.maxpg;
      ...
      const result = await getWatchlistAnime(wlid, movepg, 5, true, false, {
        towatch,
        userid:
          towatch === true ? (ctx.session.userid ?? undefined) : undefined,
      });
      ...
      for (const item of wl) {
        range
          .submenu(item.jpname, 'wl_alopts', async (ctx1) => {
            ...
            const animeData = await db.anime.findUniqueOrThrow({
