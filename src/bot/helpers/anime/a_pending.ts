...
async function animePendingBotHandle(chatid: number, userid: number, alid: number) {
    ...
    let msg = "";
    let formattedHeader = fmt`${b}Anime: ${res.jpname}${b}\n${italic}(${res.enname})\n`;
    if (res.status == "RELEASING") {
        formattedHeader = fmt`${formattedHeader}Currently airing.${italic}\n\n`;
    } else if (res.status == "FINISHED") {
        formattedHeader = fmt`${formattedHeader}Finished airing.${italic}\n\n`;
    } else {
    ...
    for (let j = 0; j < res.notwatched.length; j++) {
        if (j < 30) msg += `🔹${res.jpname} - ${res.notwatched[j]}\n`;
            ...
            const italicMsg = fmt`${italic}And many more...${italic}`;
            msg += italicMsg.text;
            break;
        }
    }
    if ((msg + formattedHeader.text).length > 1024 && res.shortname !== undefined) {
        while (msg.includes(res.jpname)) msg = msg.replace(res.jpname, res.shortname);
    }

    //Better logic
    if ((msg + formattedHeader.text).length > 1024) {
        console.log("Trying to trim");
        const lines = msg.split("\n");
        msg = lines[0] + "\n";
        for (let j = 1; j < lines.length; j++) {
            if ((formattedHeader.text + msg + lines[j]).length + 17 < 1024) msg += `${lines[j]}\n`;
            else {
                const italicMsg = fmt`${italic}And many more...${italic}`;
                msg += italicMsg.text;
                break;
    ...
    const finalMessage = fmt`${formattedHeader}${msg}`;
    await bot.api.sendPhoto(chatid, res.image, {
        caption: finalMessage.text,
