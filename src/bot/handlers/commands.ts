...
import {
  animeSearchStart,
  search_startWatch_remindMe_cb,
} from '../helpers/anime/a_search';
import {
  back_handle,
  cancel_handle,
  log_command,
...
export function botcommands() {
  bot.on('my_chat_member', async (ctx, next) => {
    if (
      ctx.myChatMember.chat.type === 'group' ||
      ctx.myChatMember.chat.type === 'supergroup'
    ) {
  ...
  bot.command(
    'deleteaccount',
    async (ctx) => await ctx.conversation.enter('deleteUser'),
  );
  ...
  bot.command(
    'markwatched',
    async (ctx) => await ctx.conversation.enter('markWatchedRange'),
  );
  ...
  bot.command(
    'createwl',
    async (ctx) => await ctx.conversation.enter('createWL'),
  );
  bot.command(
    'deletewl',
    async (ctx) => await ctx.conversation.enter('deleteWL'),
  );
  ...
  bot.callbackQuery(
    /airingupd_(\d+)(_current)?/,
    async (ctx) => await airingUpdatesListCBQ(ctx),
  );
  bot.on('callback_query:data', (ctx) =>
    ctx.answerCallbackQuery('Invalid button?'),
  ); //sink
  bot.hears(/\/start (.+)/);
  bot.hears(/^\/start$/, (ctx) => ctx.reply(`Sup boss?`));
  bot.command('help', (ctx) => ctx.reply("Help me onii-chan I'm stuck~"));
  bot.hears(/(.+)/, (ctx) =>
    ctx.reply(`What do you mean by ${ctx.match.toString()}?`),
  );
}
